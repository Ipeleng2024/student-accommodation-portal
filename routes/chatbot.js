...
function handlePaymentQuery(session) {
  ...
  return 'Your account shows ' + money(tenant.balanceDue) + ' outstanding as of ' + tenant.lastUpdated + '. If you have already paid, let management know so they can update it - this is checked manually, not automatically.';
}
...
const FALLBACK =
  'I can help with a few things: room availability, your payment status (if you are logged in as a tenant), or filing a complaint/maintenance request. What do you need?';
...
router.post('/message', (req, res) => {
  ...
  if (matches(message, ['human', 'agent', 'manager', 'speak to someone', 'call'])) {
    return res.json({
      reply: 'For anything I cannot handle, contact the office directly - add your real contact number/email in routes/chatbot.js so I can share it here.'
    });
  }
